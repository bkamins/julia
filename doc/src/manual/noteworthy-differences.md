...
For users coming to Julia from R, these are some noteworthy differences:
  ...
  * Julia performs matrix transposition using the `transpose` function and conjugated transposition using
    the `'` operator or `adjoint` function. Julia's `transpose(A)` is therefore equivalent to R's `t(A)`. Additionally
    a non-recursive transpose in Julia is provided by `permutedims` function.
  ...
  * Julia's [`sum`](@ref), [`prod`](@ref), [`maximum`](@ref), and [`minimum`](@ref) are different
    ...
    and `sum(B,2) == 12`.  If `dims` keyword argument is a vector, then it specifies all the dimensions over which
    the sum is performed, while retaining the dimensions of the summed array, e.g., `sum(A, dims=[1,2]) == hcat(10)`.
